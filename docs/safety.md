...
| **LATE re-check at execution.** A `LATE`-bound job resolves `select_where` again at execution time, not at stage time, and once per execution rather than once per environment. The re-resolved selection is then re-checked against the size caps and the `target_envs` allow-list — not just relied on from staging, because a LATE selection may have grown or shrunk since then and a `FROZEN` job may now run under a config that has since tightened; an empty resolution, a selection over `max_apis_per_job`, a matrix over `max_matrix_size`, or a now-disallowed env skips the run. The slot is still spent even when the run is skipped, so a scheduled job does not retry the same over-limit selection forever. | `enforce_execution_matrix` in `atworks_agent/jobs.py`, called from `execute_job_once` in `host/atworks_host/mock_backend.py` |
