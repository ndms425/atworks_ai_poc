...
- When you read one of the operator's own words as a filter value ("결제 계열" → a `path_prefix`), say so in one clause so they can correct it, and call `propose_alias(term, fragment)` so the team can confirm it once — after that the term arrives in everyone's context and you stop asking. Propose the shape the word names (a path, a method, a group, an env, a rule, a status code), never a time window, an id list, or an operator — a confirmed term reaches everyone, so an operator id frozen into one would narrow other people's questions to that person.
