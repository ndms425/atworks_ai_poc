...
from atworks_agent.insights import (
    KIND_LABEL,
    ROLE_PRIORITY,
    candidate_insights_from_runs,
    operator_scope,
)
...
def test_candidates_exclude_out_of_scope_apis():
    ...
    out = candidate_insights_from_runs(runs, apis, [], {"api-A"}, "developer", CFG, NOW)
...
def test_role_order_matches_priority_table():
    ...
    dev = candidate_insights_from_runs(runs, apis, jobs, None, "developer", CFG, NOW)
    qa = candidate_insights_from_runs(runs, apis, jobs, None, "qa", CFG, NOW)
...
def test_each_kind_can_be_produced():
    runs, apis, jobs = _rich_fixture()
    out = candidate_insights_from_runs(runs, apis, jobs, None, "developer", CFG, NOW)
...
def test_cap_and_deterministic_ids():
    ...
    out = candidate_insights_from_runs(runs, apis, jobs, None, "developer", small_cfg, NOW)
    again = candidate_insights_from_runs(runs, apis, jobs, None, "developer", small_cfg, NOW)
    assert len(out) == 2
    assert out == again
...
def test_scope_none_means_project_wide():
    ...
    out = candidate_insights_from_runs(runs, apis, [], None, "developer", CFG, NOW)
    ids = {c.candidate_id for c in out}
...
def test_pm_orders_stale_pending_first_and_role_tables_hold_full_order():
    ...
    pm = candidate_insights_from_runs(runs, apis, jobs, None, "pm", CFG, NOW)
...
def test_flaky_cell_label_is_humanized_but_candidate_id_keeps_the_raw_key():
    ...
    out = candidate_insights_from_runs(no_data_runs, apis, [], None, "developer", CFG, NOW)
    ...
    out2 = candidate_insights_from_runs(with_data_runs, apis, [], None, "developer", CFG, NOW)
...
def test_empty_scope_yields_no_candidates():
    runs, apis, jobs = _rich_fixture()
    empty = candidate_insights_from_runs(runs, apis, jobs, scope=set(), role="developer", config=CFG, now=NOW)
    assert empty == []
    project_wide = candidate_insights_from_runs(runs, apis, jobs, scope=None, role="developer", config=CFG, now=NOW)
    assert project_wide != []
